...
echo '<div id="lucd-manage-tickets">';
foreach ( $clients as $client ) {
    $name    = trim( $client->first_name . ' ' . $client->last_name );
    $company = trim( $client->company_name );
    $display = $company ? $company . ' - ' . $name : $name;
    echo '<div class="lucd-accordion">';
    echo '<h3 class="lucd-accordion-header" data-action="lucd_get_tickets" data-nonce="getTicketsNonce" data-client-id="' . esc_attr( $client->client_id ) . '">' . esc_html( $display ) . '</h3>';
